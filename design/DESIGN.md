...
- **Death penalty** (implemented in `strike_enemy()`): dying to a pack costs 25% of current gold and 10% of current xp, taken off whatever the player had the instant they died (never off a reward — pack wipes are win-only, see §4b). This is the counterweight to how freely Number of Enemies Spawned / Number of Affixes / Player Debuffs / Number of Banishments can be pushed (see §4c in `schema.sql`): those knobs have no ceiling and reward proportionally harder fights, so without a real cost for losing there'd be no reason not to max every one of them and grind through deaths for free. TUNE the two percentages once playtested.
...
## 4b. Solo combat (Test Rat)

A standalone player-vs-mob loop (`enemies`, `player_combat`, `strike_enemy()`) drives the "Current Battle" panel independently of guild bosses — currently just one always-available test enemy so the panel and action-gating have something to exercise. Each idle tick auto-strikes the current enemy (1 action per strike); defeating it respawns a fresh one immediately and fully heals the player, so there's always a fight in progress with no death penalty yet. This is the seed of what a real mob/dungeon system will replace.
